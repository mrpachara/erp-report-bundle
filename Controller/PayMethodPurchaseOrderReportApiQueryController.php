...
/**
 * PayMethod PurchaseOrder Report Api Controller
 *
 * @Rest\Version("1.0")
 * @Rest\Route("/api/report/pay-method-purchase-order")
 * @Rest\View(serializerEnableMaxDepthChecks=true)
 */
class PayMethodPurchaseOrderReportApiQueryController
{
    /** 
    ...
    /**
     * PayMethodPurchaseOrderReportApiQueryController constructor.
     */
    ...
    public function payMethodPurchaseOrderSummaryExportAction(ServerRequestInterface $request, string $format)
    {
        ...
        switch(strtolower($format)) {
            case 'pdf':
                $view = $this->templating->render('@ErpReport/pdf/pay-method-purchase-order-report.pdf.twig', [
                    'profile' => $profile,
                    'model' => $data,
            ...
            case 'xlsx':
                ...
                $sheet->setCellValue('A1', 'รายงานการชำระเงิน โดย ใบสั่งซื้อ (Payment by PO-FI)');
                ...
                $fileName = 'RP-FI-PU-PO-Payment_rev.2.1.0_'.date('Ymd_His', time()).'.xlsx';
