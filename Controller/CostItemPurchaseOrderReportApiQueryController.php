...
use Erp\Bundle\ReportBundle\Authorization\CostItemPurchaseOrderReportAuthorization;
...
class CostItemPurchaseOrderReportApiQueryController
{
    ...
    /**
     * @var CostItemPurchaseOrderReportAuthorization
     */
    ...
    public function __construct(
        ...
        CostItemPurchaseOrderReportAuthorization $authorization
    ) {
    ...
    {
        return $this->reduce(
            $this->domainQuery->costItemGroupPurchaseOrderSummary($filter, $filterDetail)
        );
    }
    ...
    {
        return $this->reduce(
            $this->domainQuery->costItemDistributionPurchaseOrderSummary($filter, $filterDetail)
        );
    }
     ...
     */
    public function costItemDistributionPurchaseOrderSummaryExportAction(ServerRequestInterface $request, string $format)
    {
        $filterDetail = [];
        ...
        switch (strtolower($format)) {
            ...
            case 'xlsx':
                ...
                $sheet->setCellValue('A1', 'รายงานปริมาณสินค้า โดย ใบสั่งซื้อ (CI-Quantity by PO)');
                ...
                $fileName = 'RP-MT-CI-Quantity-PO_rev.2.1.0_' . date('Ymd_His', time()) . '.xlsx';
     ...
     */
    public function costItemDistributionPurchaseOrderPriceSummaryExportAction(ServerRequestInterface $request, string $format)
    {
        $filterDetail = [];
        $data = $this->getDistribution($request->getQueryParams(), $filterDetail);
        $profile = $this->settingQuery->findOneByCode('profile')->getValue();
        ...
        switch (strtolower($format)) {
            ...
            case 'xlsx':
                ...
                $sheet->getStyle('C7:C8')->getAlignment()->setHorizontal('right');
                $sheet->getStyle('D7:D8')->getNumberFormat()->setFormatCode('DD/MM/YYYY');
                $sheet->setCellValue('A1', 'รายงานมูลค่าสินค้า โดย ใบสั่งซื้อ (CI-Price by PO)');
                $sheet->setCellValue('A2', 'โครงการ : ');
                $sheet->setCellValue('A3', 'งบประมาณ : ');
                $sheet->setCellValue('A4', 'ประเภท : ');
                $sheet->setCellValue('A5', 'ผู้ต้องการ : ');
                ...
                // Create a Temporary file in the system
                $fileName = 'RP-MT-CI-Price-PO_rev.2.1.0_' . date('Ymd_His', time()) . '.xlsx';
                $temp_file = tempnam(sys_get_temp_dir(), $fileName);
     ...
     */
    public function costItemGroupPurchaseOrderSummaryExportAction(ServerRequestInterface $request, string $format)
    {
        $filterDetail = [];
     ...
     */
    public function costItemGroupPurchaseOrderPriceSummaryExportAction(ServerRequestInterface $request, string $format)
    {
        $filterDetail = [];
        $data = $this->getGroup($request->getQueryParams(), $filterDetail);
