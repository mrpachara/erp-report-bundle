...
use Erp\Bundle\ReportBundle\Domain\CQRS\CostItemExpenseReportQuery as QueryInterface;

class CostItemExpenseReportQueryService implements QueryInterface
{
    ...
    function costItemGroupExpenseQueryBuilder($alias)
    {
        ...
        $qb
            ->select("{$alias}_costItem.code AS code")
                ...
                ->addSelect("{$alias}_purchase.code AS expenseCode")
    ...
    {
        $filterDetail = [];
        $qb = $this->costItemGroupExpenseQueryBuilder('_entity');
    ...
    function costItemDistributionExpenseQueryBuilder($alias)
    {
        $qb = $this->repository->createQueryBuilder($alias);
        $qb
            ->select("{$alias}_costItem.code AS code")
    ...
    {
        $filterDetail = [];
        $qb = $this->costItemDistributionExpenseQueryBuilder('_entity');
        if(!empty($filter['start'])) {
            $qb
                ->andWhere('_entity_purchase.tstmp >= :startDate')
                ->setParameter('startDate', new \DateTimeImmutable($filter['start']))
