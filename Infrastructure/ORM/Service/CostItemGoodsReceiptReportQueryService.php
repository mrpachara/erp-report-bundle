...
use Erp\Bundle\ReportBundle\Domain\CQRS\CostItemGoodsReceiptReportQuery as QueryInterface;

class CostItemGoodsReceiptReportQueryService implements QueryInterface
{
    ...
    {
        $this->repository = $doctrine->getRepository('ErpDocumentBundle:GoodsReceiptDetail');
    ...
    function costItemGroupGoodsReceiptQueryBuilder($alias)
    {
    ...
    function costItemDistributionGoodsReceiptQueryBuilder($alias)
    {
        $qb = $this->repository->createQueryBuilder($alias);
        $qb
            ->select("{$alias}_costItem.code AS code")
