...
use Erp\Bundle\ReportBundle\Domain\CQRS\ProjectBoqPurchaseOrderReportQuery as QueryInterface;
use Erp\Bundle\MasterBundle\Entity\ProjectBoq;
use Erp\Bundle\MasterBundle\Entity\ProjectBoqBudgetType;
class ProjectBoqPurchaseOrderReportQueryService implements QueryInterface
{
                ...
                foreach($result['cost']['columns'] as $column) {
                    if($column['id'] !== null) {
                        $costData['costs'][] = [
                            ...
                            'cost' => (double)$boq->getBudgets()[$column['id']]->cost['order']['approved'] + $boq->getBudgets()[$column['id']]->cost['order']['nonapproved']
                            + $boq->getBudgets()[$column['id']]->cost['expense']['approved'] + $boq->getBudgets()[$column['id']]->cost['expense']['nonapproved'],
                            'remain' => (double)($boq->getBudgets()[$column['id']]->getBudget() - ($boq->getBudgets()[$column['id']]->cost['order']['approved'] + $boq->getBudgets()[$column['id']]->cost['order']['nonapproved']
                                + $boq->getBudgets()[$column['id']]->cost['expense']['approved'] + $boq->getBudgets()[$column['id']]->cost['expense']['nonapproved'])),
                        ];
                    }
                }
