...
use Erp\Bundle\ReportBundle\Domain\CQRS\ProjectBoqExpenseReportQuery as QueryInterface;
class ProjectBoqExpenseReportQueryService implements QueryInterface
{
                ...
                foreach($result['cost']['columns'] as $column) {
                    if($column['id'] !== null) {
                        $costData['costs'][] = [
                            ...
                            'cost' => (double)$boq->getBudgets()[$column['id']]->cost['expense']['approved'] + $boq->getBudgets()[$column['id']]->cost['expense']['nonapproved'],
                            'remain' => (double)($boq->getBudgets()[$column['id']]->getBudget() - ($boq->getBudgets()[$column['id']]->cost['expense']['approved'] + $boq->getBudgets()[$column['id']]->cost['expense']['nonapproved'])),
                        ];
                    }
                }
